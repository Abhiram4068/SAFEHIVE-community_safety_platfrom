...
export async function PATCH(
  ...
  const postId = slug; // slug contains the post ID
  
  console.log("Save route called with slug/postId:", postId);
  ...
  try {
    console.log(`Making request to: ${POST_SERVICE_URL}/api/post/${postId}/save/`);
    
    // Forward the request with the cookies in the headers
    const response = await axios.patch(
      `${POST_SERVICE_URL}/api/post/${postId}/save/`,
      {}, 
      {
        headers: {
