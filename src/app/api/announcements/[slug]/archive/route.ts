...
export async function PATCH(
  ...
  try {
    console.log(`Making request to: ${ANNOUNCEMENT_SERVICE_URL}/api/announcements/${postId}/archive/`);
    
    // Forward the request with the cookies in the headers
    const response = await axios.patch(
      `${ANNOUNCEMENT_SERVICE_URL}/api/announcements/${postId}/archive/`,
      {},
      {
        headers: {
    ...
    console.log("Django announcement response:", response.data);
