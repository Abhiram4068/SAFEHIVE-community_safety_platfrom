...
const EditProfilePage = ({ profileData }: any) => {
  ...
  const [previewUrl, setPreviewUrl] = useState<string>(profileData?.profile_image || "");
  ...
  const handleSave = async () => {
    ...
    try {
      // Calling the local API route created in Step 1
      const response = await axios.patch("/api/userprofile/update", submissionData, {
      ...
      const errorMsg = isHtml ? "API Route Not Found. Check your folder structure." : JSON.stringify(error.response?.data || "Error");
            ...
            <div className="flex justify-between items-end -mt-12 mb-8">
              <div className="relative group w-32 h-32">
                <div className="w-full h-full bg-[#1A1A1B] rounded-3xl border-[6px] border-[#0B0D10] shadow-2xl overflow-hidden relative">
                    {previewUrl ? <img src={previewUrl} alt="avatar" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-tr from-orange-500 to-yellow-400" />}
                </div>
                <label className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 rounded-3xl opacity-0 group-hover:opacity-100 cursor-pointer">
                    <Camera size={24} /><span className="text-[10px] font-black uppercase mt-1">Upload</span>
                    <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                </label>
              </div>

              <div className="flex gap-3 mb-2">
                <button disabled={isSaving} onClick={handleSave} className="flex items-center gap-2 bg-orange-600 text-white font-bold px-6 py-2 rounded-full text-sm hover:bg-orange-700 transition-colors disabled:opacity-50 min-w-[140px] justify-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input type="text" name="display_name" value={formData.display_name} onChange={handleChange} className="w-full bg-[#16191D] border border-[#1F2228] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 transition-colors" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-gray-500 ml-1">About (Bio)</label>
                  <textarea name="bio" rows={4} value={formData.bio} onChange={handleChange} className="w-full bg-[#16191D] border border-[#1F2228] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-500 transition-colors resize-none" />
              ...
              <div className="bg-[#16191D]/50 border border-[#1F2228] rounded-2xl p-6 border-l-orange-600/50 border-l-2 h-fit">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">Max file size is 5MB. Supported formats: JPG, PNG, WEBP.</p>
              </div>
            </div>
      ...
      {(showDisableModal || showDeleteModal) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#0B0D10] border border-[#1F2228] w-full max-w-md rounded-2xl p-6 shadow-2xl text-white">
            <h2 className="text-xl font-bold mb-6">{showDeleteModal ? "Delete Account" : "Disable Account"}</h2>
            <div className="flex gap-3">
              <button onClick={() => {setShowDisableModal(false); setShowDeleteModal(false)}} className="flex-1 px-5 py-3 rounded-xl text-sm font-bold bg-[#16191D]">Cancel</button>
              <button className={`flex-1 px-5 py-3 rounded-xl text-sm font-bold ${showDeleteModal ? 'bg-red-600' : 'bg-orange-600'}`}>Confirm</button>
            </div>
          </div>
        </div>
      )}
