...
export default function CommunityCenter({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleDismantleConfirm = async () => {
    ...
    try {
      // This calls your Next.js API route which proxies to Django
      await axios.post(`/api/community/dismantle/`,  { groupId  });
      window.location.href = '/communities'; // Redirect after deletion
    } catch (error) {
    ...
    <div className="flex-1 bg-black min-h-screen pb-10">
          ...
          <div className="relative bg-[#1A1D23] border border-[#343536] w-full max-w-sm rounded-2xl p-6 shadow-2xl">
            {!joinedSuccess ? (
              <>
                ...
                <p className="text-[#818384] text-sm mb-6">Are you sure you want to join <span className="text-white font-semibold">r/{communityName}</span>?</p>
      ...
      <div className="w-full">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-10 flex justify-between items-center">
          <h1 className="text-white text-3xl font-bold tracking-tighter uppercase">r/{communityName}</h1>
          
          <div className="flex items-center gap-3">
            {/* Admin Dismantle Button */}
            {isAdmin && (
              <button 
                onClick={() => setIsDismantleModalOpen(true)}
                className="border border-red-600/50 text-red-500 px-4 py-1.5 rounded-full font-bold text-xs hover:bg-red-500/10 transition"
              >
                Dismantle
              </button>
            )}
            ...
            {isJoined ? (
              ...
              <button onClick={() => setIsModalOpen(true)} className="bg-white hover:bg-gray-200 text-black px-6 py-1.5 rounded-full font-bold text-sm transition">Join</button>
            )}
          </div>
        </div>
